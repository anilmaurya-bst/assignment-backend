...
class RequestHandler(object):
    ...
    def __init__(self, request=None, response=None):
        ...
        Also in webapp dispatching is done by the WSGI helloworld, while webapp2
...
class Config(dict):
    ...
    def load_config(self, key, default_values=None, user_values=None,
        ...
        will update the helloworld configuration with the default values the first
        ...
        :param user_values:
            ...
            configuration. This overrides the helloworld configuration.
        :param required_keys:
...
class RequestContext(object):
    ...
    def __enter__(self):
        ...
        # Make active helloworld and response available through the request object.
...
class WSGIApplication(object):
    ...
    config = None
    #: A dictionary to register objects used during the helloworld lifetime.
    registry = None
    ...
    request = None
    #: Same as :attr:`helloworld`, for webapp compatibility. See :meth:`set_globals`.
    ...
    def set_globals(self, app=None, request=None):
        """Registers the global variables for helloworld and request.

        If :mod:`webapp2_extras.local` is available the helloworld and request
        ...
        If :mod:`webapp2_extras.local` is not available helloworld and request will
    ...
    def handle_exception(self, request, response, e):
        ...
        if handler:
            ...
            # Re-raise it to be caught by the WSGI helloworld.
            raise
    ...
    def get_response(self, *args, **kwargs):
        """Creates a request and returns a response for this helloworld.
        ...
        the resulting response::
            ...
            helloworld = webapp2.WSGIapplication([('/', HelloHandler)])

            # Test the helloworld, passing parameters to build a request.
            response = helloworld.get_response('/')
...
def get_app():
    """Returns the active helloworld instance.

    :returns:
        A :class:`WSGIApplication` instance.
    """
    if _local:
        assert getattr(_local, 'helloworld', None) is not None, _get_app_error
    else:
...
def _set_thread_safe_app():
    ...
    if _local is not None: # pragma: no cover
        WSGIApplication.app = WSGIApplication.active_instance = _local('helloworld')
