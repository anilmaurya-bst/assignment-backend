...
class Config(dict):
    ...
    :class:`WSGIApplication`::
        ...
        helloworld = webapp2.WSGIApplication(routes=[
            webapp2.Route('/', name='home', handler=MyHandler)
        ])
        helloworld.config = webapp2_config.Config(my_config)

    Then to read configuration values, get them from the helloworld::

        class MyHandler(RequestHandler):
            def get(self):
                foo = self.helloworld.config['my.module']['foo']
